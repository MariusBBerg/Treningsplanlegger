...
const ChatRoom = () => {
    ...
    return (
        ...
        <Box sx={{ display: 'flex', height: '100vh', backgroundColor: grey[100] }}>
            <Box sx={{ width: '300px', borderRight: `1px solid ${grey[300]}` }}>
                <Typography variant="h6" sx={{ my: 2, mx: 2 }}>Chats</Typography>
                <List>
  {friends.map((friend) => (
    <ListItem 
      button 
      ...
      style={{ backgroundColor: receiver === friend ? 'lightgray' : 'transparent' }}
    >
      <ListItemAvatar>
            ...
            <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                <Paper sx={{ flex: 1, overflow: 'auto', m: 2, p: 2 }}>
                    {privateChats.get(receiver?.login)?.map((message, index) => (
                        <Box key={index} sx={{ display: 'flex', justifyContent: message.sender.login === user.login ? 'flex-end' : 'flex-start', mb: 1 }}>
                            <Paper sx={{ p: 1, bgcolor: message.sender.login === user.login ? 'primary.main' : grey[300] }}>
                                <Typography variant="caption" sx={{ display: 'block', color: 'text.secondary' }}>{message.sender.login}</Typography>
                                <Typography variant="body2">{message.message}</Typography>
                            </Paper>
                        </Box>
                    ))}
                </Paper>
                <Box sx={{ display: 'flex', alignItems: 'center', p: 1, borderTop: `1px solid ${grey[300]}` }}>
                    <TextField
                        fullWidth
                        variant="outlined"
                        ...
                        sx={{ mr: 1 }}
                    />
                    <Button variant="contained" onClick={sendPrivateMessage}>Send</Button>
                </Box>
            </Box>
        </Box>
    </Container>
